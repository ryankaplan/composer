  ...
  implements ReadOnlyObservable<T>
{
  ...
  constructor(private _value: T, private equals?: (a: T, b: T) => boolean) {
    super();
  }
...
export class PersistedObservable<T> extends Observable<T> {
  constructor(key: string, initialValue: T, migrations?: MigrationChain<T>) {
    super(initialValue);
    this.runMigrationsAndSetInitialValue(key, migrations);
  ...
  runMigrationsAndSetInitialValue(
    ...
    if (!migrations) {
      if (currentValue !== null) {
        this.set(JSON.parse(currentValue));
      }
      return;
    }
